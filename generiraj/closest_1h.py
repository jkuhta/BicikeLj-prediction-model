...
times2 = train["timestamp"].values - pd.Timedelta(hours=1)
ptimes = test["timestamp"].values - pd.Timedelta(hours=1)
ptimes2 = test["timestamp"].values - pd.Timedelta(hours=1)
...
for i, t in enumerate(ptimes):
    ...
    if abs(times[closest] - t) > pd.Timedelta(hours=1):
...
# test.drop('timestamp2', axis=1)
# train2.drop('timestamp2', axis=1)
train2.to_csv("closest_1h_train.csv", sep=",", index=False)
test.to_csv("closest_1h_test.csv", sep=",", index=False)