...
times2 = train["timestamp"].values - pd.Timedelta(hours=3)
ptimes = test["timestamp"].values - pd.Timedelta(hours=3)
ptimes2 = test["timestamp"].values - pd.Timedelta(hours=3)
...
for i, t in enumerate(ptimes):
    ...
    if abs(times[closest] - t) > pd.Timedelta(hours=3):
...
train2.to_csv("closest_3h_train.csv", sep=",", index=False)
test.to_csv("closest_3h_test.csv", sep=",", index=False)